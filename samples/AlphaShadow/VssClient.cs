...
{
   public sealed class VssClient : IDisposable
   {
      #region Private Fields

      private List<VssWriterDescriptor> m_writerComponentsForRestore;
      private static IVssFactory s_implementation;
      ...
      {
         Host = host ?? throw new ArgumentNullException(nameof(host), $"{nameof(host)} is null.");
      }
      ...
      public static IVssFactory Implementation
      {
         get
         {
            if (s_implementation == null)
            {
               s_implementation = VssFactoryProvider.Default.GetVssFactory();
            }
      ...
      public async Task CreateSnapshotAsync(IEnumerable<string> volumeList, string outputXmlFile, IEnumerable<string> excludedWriterList, IEnumerable<string> includedWriterList)
      {
         if (volumeList == null)
            throw new ArgumentNullException(nameof(volumeList), $"{nameof(volumeList)} is null.");

         if (excludedWriterList == null)
            throw new ArgumentNullException(nameof(excludedWriterList), $"{nameof(excludedWriterList)} is null.");

         if (includedWriterList == null)
            throw new ArgumentNullException(nameof(includedWriterList), $"{nameof(includedWriterList)} is null.");
         ...
         if (snapshotWithWriters)
         {
            ...
            await GatherWriterMetadataAsync().ConfigureAwait(false);
         ...
         if (snapshotWithWriters)
            await PrepareForBackupAsync().ConfigureAwait(false);

         // Create the shadow set 
         await DoSnapshotSetAsync().ConfigureAwait(false);
      ...
      private async Task DoSnapshotSetAsync()
      {
         await Host.WaitForTaskAsync(ct => m_backupComponents.DoSnapshotSetAsync(ct), "Creating the shadow (DoSnapshotSet)...").ConfigureAwait(false);
         ...
         await CheckSelectedWriterStatusAsync().ConfigureAwait(false);

         Host.WriteLine("Shadow copy set successfully created.");
      }

      private async Task PrepareForBackupAsync()
      {
      ...
      internal async Task CheckSelectedWriterStatusAsync()
      {
         ...
         await GatherWriterStatusAsync().ConfigureAwait(false);
      ...
      public async Task GatherWriterMetadataAsync()
      {
         // Gathers writer metadata
         // WARNING: this call can be performed only once per IVssBackupComponents instance!
         await Host.WaitForTaskAsync(ct => m_backupComponents.GatherWriterMetadataAsync(ct), "Gathering writer metadata...").ConfigureAwait(false);

         Host.WriteLine("- Initialize writer metadata...");
         InitializeWriterMetadata();
      }

      internal async Task GatherWriterMetadataToScreenAsync()
      {
         await Host.WaitForTaskAsync(ct => m_backupComponents.GatherWriterMetadataAsync(ct), "Gathering writer metadata...").ConfigureAwait(false);
      ...
      public async Task GatherWriterStatusAsync()
      {
         await Host.WaitForTaskAsync(ct => m_backupComponents.GatherWriterStatusAsync(ct), "Gather writer status...").ConfigureAwait(false);
      }
      ...
      internal async Task BackupCompleteAsync(bool succeeded)
      {
      ...
      internal async Task RevertToSnapshotAsync(Guid snapshotId)
      {
         ...
         if (s_implementation.GetInfoProvider().ShouldBlockRevert(snapshot.OriginalVolumeName))
         {
         ...
         await Host.WaitForTaskAsync(ct => m_backupComponents.QueryRevertStatusAsync(snapshot.OriginalVolumeName, ct)).ConfigureAwait(false);
      ...
      {
         Host.WaitForTaskAsync(ct => m_backupComponents.ImportSnapshotsAsync(ct), "Importing the transportable shadow copy...");
